		...
		case CONNECTED:
			...
			while(1) {
				if(servertcb->usedBufLen<length) {
					...
					char* dest = (char*) buf;
					memmove(dest,servertcb->recvBuf,length);
					memmove(servertcb->recvBuf, servertcb->recvBuf+length,servertcb->usedBufLen-length);
...
int savedata(svr_tcb_t* svrtcb, seg_t* segment) {
	if(segment->header.length + svrtcb->usedBufLen < RECEIVE_BUF_SIZE) {
		...
		memmove(&svrtcb->recvBuf[svrtcb->usedBufLen],segment->data,segment->header.length);
